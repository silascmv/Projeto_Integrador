      ...
      pool.beginTransaction(function(err) {
        ...
        pool.query(query, function(error, results, fields) {
          if (!error) {
            ...
            pool.query(query, function(error, results, fields) {
              ...
              if (!error) {
                pool.commit(function(err) {
                  ...
                  if (!err) {
                    var resultado_cliente = JSON.parse(
                      '{"status":"Cliente Cadastrado com Sucesso","code_status":"01"}'
                    );
                    ...
                    return pool.rollback(function() {
                      var resultado_cliente = JSON.parse(
                        '{"status":"Falha ao realizar o cadastro tenta novamente","code_status":"00"}'
                      );
                      resolve(resultado_cliente);
                    });
                  }
                ...
                return pool.rollback(function() {
                  var resultado_cliente = JSON.parse(
                    '{"status":"Já existe um Cliente com esse E-mail Cadastrado","code_status":"02"}'
                  );
                  resolve(resultado_cliente);
                });
                ...
                var resultado = JSON.parse(
                  '{"status":"Coluna com o valor maior do que o permitido","code_status":"03"}'
                );
                resolve(resultado);
              } else {
                var resultado = JSON.parse(
                  '{"status":"Operação desconhecida, entre em contato com TI","code_status":"00"}'
                );
                resolve(resultado);
              }
            });
            ...
            return pool.rollback(function() {
              var resultado = JSON.parse(
                '{"status":"Já existe um usuário com esse Login","code_status":"04"}'
              );
              resolve(resultado);
            });
          } else {
  ...
  insertProduto(req, pool, objeto_produto) {
    return new Promise((resolve, reject) => {
      ...
      pool.beginTransaction(function(err) {
        ...
        pool.query("INSERT INTO PRODUTOS SET ?", objeto_produto, function(
          error,
          results,
          fields
        ) {
          if (!error) {
            pool.commit(function(err) {
              ...
              if (!err) {
                var resultado_cliente = JSON.parse(
                  '{"status":"Produto Cadastrado com Sucesso","code_status":"01"}'
                );
                resolve(resultado_cliente);
              } else {
                //Caso apresente algua falha no commit irá enviar essa informação.

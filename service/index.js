...
app.post("/addCliente", function(req, res) {
  ...
  pool.getConnection(function(err, pool) {
    ...
    try {
      ...
      var error_status = JSON.parse(
        '{"status":"Não foi possível realizar sua operação, entre em contato com o administrador.","code_status":"00"}'
      );
...
app.get("/realizarLogin/:login&:password", function(req, res) {
  pool.getConnection(function(err, pool) {
    ...
    if (req.params.password)
      filter_password = "AND CD_SENHA =" + "'" + req.params.password + "'";
    // Executando a query MySQL (selecionar todos os dados da tabela usuário).
    pool.query(
      "SELECT CD_LOGIN,CD_SENHA FROM LOGIN WHERE CD_LOGIN = " +
        filter_login +
        filter_password,
      function(error, results, fields) {
        ...
        if (isEmptyObject(results)) {
          res.json({ status: "Usuario ou Senha Invalido", code_status: 0 });
          pool.release();
          pool.destroy();
        } else {
          res.json({ status: "Login Realizado com Sucesso", code_status: 1 });
          pool.release();
          pool.destroy();
        }
      }
...
app.post("/listarProdutoId/", (req, res) => {
  pool.getConnection((err, pool) => {
    var id = req.param("id");
    var query =
      "SELECT NOME_PRODUTO,VALOR,DESCRICAO,IMAGEM FROM PRODUTOS WHERE ID_PRODUTO =" +
      id;
    pool.query(query, (error, results, fields) => {
      ...
      async function retrieveFoto() {
        await Image_Converter(
          results[0].IMAGEM,
          results[0].NOME_PRODUTO + ".jpg"
        ).then(resolve => {
          var objeto_json = {};

          res.sendFile(
            __dirname + "/" + results[0].NOME_PRODUTO + ".jpg",
            "Sucesso"
          );
        });
      }
